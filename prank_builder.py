        ...
        try:
            ...
            payload_path.write_text(
                generate_payload(
                    ...
                    monitor_indexes=list(self.config.get("monitor_indexes") or [0]),
...
class MainWindow(QMainWindow):
    ...
    def build_settings_tab(self) -> QWidget:
        ...
        row = 0
        row = self.add_settings_row(grid, row, "Mode", self.mode_combo)
        row = self.add_settings_row(grid, row, "Topmost", self.topmost_check)
        row = self.add_settings_row(grid, row, "Window behavior", self.fullscreen_check)
        row = self.add_settings_row(grid, row, "Sound behavior", self.loop_sound_check)
        row = self.add_settings_row(grid, row, "Volume", self.max_volume_check)
        row = self.add_settings_row(grid, row, "Pre-roll", self.relax_mode_check)
        row = self.add_settings_row(grid, row, "Exit options", self.esc_exit_check)
        row = self.add_settings_row(grid, row, "Delay before showing", self.delay_spin)
        row = self.add_settings_row(grid, row, "Window Size", slider_widget)
        self.add_settings_row(grid, row, "Payload title", self.title_field)

        monitor_group = QGroupBox("Target Monitors")
        monitor_layout = QVBoxLayout(monitor_group)
        monitor_layout.setContentsMargins(18, 24, 18, 18)
        monitor_layout.setSpacing(10)
        self.monitor_hint_label = QLabel("Select one or more monitors for the prank windows.")
        self.monitor_hint_label.setWordWrap(True)
        monitor_layout.addWidget(self.monitor_hint_label)
        self.monitor_checks_container = QWidget()
        self.monitor_checks_layout = QVBoxLayout(self.monitor_checks_container)
        self.monitor_checks_layout.setContentsMargins(0, 0, 0, 0)
        self.monitor_checks_layout.setSpacing(8)
        monitor_layout.addWidget(self.monitor_checks_container)
        self.refresh_monitor_checkboxes()

        safety_group = QGroupBox("Safety Summary")
        safety_layout = QVBoxLayout(safety_group)
        safety_layout.setContentsMargins(18, 24, 18, 18)
    ...
    def build_help_tab(self) -> QWidget:
        ...
        help_panel.setHtml(
            ...
            <p><span id='helpTitle'>Target monitors</span><br>
            Select one or more monitor checkboxes. The generated prank opens one window per selected monitor and positions each window relative to that screen.</p>
    ...
    def add_settings_row(self, layout: QGridLayout, row: int, label_text: str, widget: QWidget) -> int:
    ...
    def available_monitor_entries(self) -> list[tuple[int, str]]:
        screens = QApplication.screens()[:4]
        ...
        return entries or [(0, "Monitor 1 (Primary)")]

    def refresh_monitor_checkboxes(self) -> None:
        self.monitor_checks: list[QCheckBox] = []
        while self.monitor_checks_layout.count():
            item = self.monitor_checks_layout.takeAt(0)
            widget = item.widget()
            if widget is not None:
                widget.deleteLater()
        for index, label in self.available_monitor_entries():
            checkbox = QCheckBox(label)
            checkbox.setProperty("monitor_index", index)
            checkbox.setChecked(index == 0)
            self.monitor_checks_layout.addWidget(checkbox)
            self.monitor_checks.append(checkbox)
        self.monitor_checks_layout.addStretch(1)

    def selected_monitor_indexes(self) -> list[int]:
        return [
            int(checkbox.property("monitor_index"))
            for checkbox in getattr(self, "monitor_checks", [])
    ...
    def collect_config(self) -> dict[str, object]:
        return {
            ...
            "monitor_indexes": self.selected_monitor_indexes(),
    ...
    def apply_config(self, config: dict[str, object]) -> None:
        ...
        selected_monitors = {int(item) for item in config.get("monitor_indexes") or [0]}
        for checkbox in getattr(self, "monitor_checks", []):
            checkbox.setChecked(int(checkbox.property("monitor_index")) in selected_monitors)
    ...
    def start_build(self) -> None:
        ...
        if not valid:
            ...
            self.show_message(APP_TITLE, message, "warning")
            return
        if not self.selected_monitor_indexes():
            message = "Select at least one target monitor before building."
            self.log(message, "error")
            self.show_message(APP_TITLE, message, "warning")
            return
...
def generate_payload(
    ...
    data = {
        ...
        "monitors": base64.b64encode(json.dumps(monitor_indexes).encode("utf-8")).decode("ascii"),
    }
...
MONITOR_INDICES = json.loads(base64.b64decode("{data['monitors']}").decode("utf-8"))
TOPMOST = {topmost!r}
...
class PayloadWindow(QWidget):
    ...
    def apply_scaled_window_size(self) -> None:
        ...
        width = int(screen_geo.width() * scale)
        height = int(screen_geo.height() * scale)
    ...
    def configure_sound(self) -> None:
        if not self.sound_path or self.player is not None:
            return
        self.audio_output = QAudioOutput(self)
    ...
    def scale_pixmap(self) -> None:
        ...
        scaled = self.pixmap.scaled(target_size, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
...
def resolved_screens() -> list:
    screens = QGuiApplication.screens()
    selected = [screens[index] for index in MONITOR_INDICES if 0 <= index < len(screens)]
    return selected or [QGuiApplication.primaryScreen()]


app = QApplication(sys.argv)
screens = resolved_screens()
prank_windows = [PayloadWindow(screen) for screen in screens]
relax_windows = [RelaxWindow(screen) for screen in screens] if RELAX_MODE else []


def quit_all() -> None:
    for relax in relax_windows:
        relax.close()
    for prank in prank_windows:
        if prank.player:
            prank.player.stop()
        if prank.movie:
            prank.movie.stop()
...
for prank in prank_windows:
    attach_exit_shortcuts(prank, quit_all)
for relax in relax_windows:
    attach_exit_shortcuts(relax, quit_all)


def start_audio() -> None:
    for prank in prank_windows:
        prank.configure_sound()


def reveal_windows() -> None:
    for prank in prank_windows:
        prank.show()
        if MODE == 'Intensive':
            prank.raise_and_focus()
    QTimer.singleShot(100, start_audio)


def hide_relax_and_reveal() -> None:
    for relax in relax_windows:
        relax.close()
    reveal_windows()
...
if RELAX_MODE:
    ...
    for relax in relax_windows:
        relax.show()
    QTimer.singleShot(max(DELAY_SECONDS * 1000, 1200), hide_relax_and_reveal)
elif DELAY_SECONDS > 0:
    for prank in prank_windows:
        prank.hide()
    QTimer.singleShot(DELAY_SECONDS * 1000, reveal_windows)
else:
    reveal_windows()
sys.exit(app.exec())
'''
